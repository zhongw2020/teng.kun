...
@Component({
  ...
  templateUrl: './out-coudan.component.html',
  styles: []
  

})
export class OutCouDanComponent extends STComponentBase implements OnInit {
    ...
    let columns: OsharpSTColumn[] = [
      {
        title: '操作', fixed: 'left', width: 65, buttons: [{
          text: '操作', children: [
            { text: '编辑', icon: 'edit', acl: 'Root.Admin.OutStorManager.OutStor.Update',  click: row => this.edit(row) },
      ...
      {
        title: '单据类型', index: 'OutstorCategory', sort: true, editable: true, filterable: true, filter: { default:true,key: "OutstorCategory", menus: [{ text: '凑单', value: '凑单', checked: true }] } ,ftype: 'string', ui: { grid: { span: 24 } } },
      { title: '凑单凭证号', index: 'CDOutstorVoucher', sort: true, readOnly: true, editable: true, filterable: true, ftype: 'string' },
      { title: '实际凭证号', index: 'OutstorVoucher', sort: { key: 'Id', default: "descend" }, readOnly: true, editable: true, filterable: true, ftype: 'string' },
